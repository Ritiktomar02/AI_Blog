...
const extractText = (aiResponse) => {
  if (!aiResponse?.content?.parts) return "";
  return aiResponse.content.parts.map((p) => p.text).join("\n\n");
};
...
const GenerateBlogPostForm = ({
  ...
  const handleGenerateBlogPost = async (e) => {
    ...
    try {
      ...
      // ensure we only pass plain markdown text to the editor
      const extractedContent = extractText(generatedPost);
